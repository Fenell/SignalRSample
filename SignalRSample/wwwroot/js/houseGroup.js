...
//Connect with method to Hub

connectionHouse.on("newMemberAddHouse", (houseName)=>{
    toastr.success("Member has subscribed from "+ houseName);
});

connectionHouse.on("newMemberRemoveHouse",(houseName)=>{
...
connectionHouse.on("subscriptionStatus", (strGroupJoined, houseName, hasSubscribed) => {
    lbl_houseJoined.innerText = strGroupJoined.toString();
...
function successful(){
    console.log("Connect to House Group Hub successfully");
}
...
connectionHouse.start().then(successful, rejected);